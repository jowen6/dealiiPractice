...
#ifndef slepc_solver_MOD_h
#  define slepc_solver_MOD_h
...
namespace SLEPcWrappers
{
  ...
  public:
    ...
    solve(const PETScWrappers::MatrixBase &A,
          ...
          std::vector<PetscScalar> &       eigenvalues,
          std::vector<OutputVector> &      eigenvectors,
          const unsigned int               n_eigenpairs); //REMOVED = 1 to distinguish from MOD
    ...
    //MOD TO ALLOW INTERVAL SPECIFICATION use with set_which_eigenpairs(EPS_ALL)
    void
    set_target_interval(const PetscReal &left_bound, const PetscReal &right_bound);
    /**
  ...
  protected:
    ...
    void
    solve(unsigned int *n_converged);
  ...
  protected:
     ...
     */
    EPS eps;
    ST  st; //spectral transformaion object
    KSP ksp; //Not sure
                    ...
                    const unsigned int               n_eigenpairs)
  {
                      ...
                      std::vector<OutputVector> &      eigenvectors)
    {
        ...
        eigenvectors.resize(n_converged, eigenvectors.front());
        eigenvalues.resize(n_converged);
        std::cout << "  Number Converged Eigenvalues: " << n_converged << "\n";
        for (unsigned int index = 0; index < n_converged; ++index)
            get_eigenpair(index, eigenvalues[index], eigenvectors[index]);
    }
