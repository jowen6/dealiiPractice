...
namespace SLEPcWrappers
{
    ...
    , reason(EPS_CONVERGED_ITERATING)
  {
    ...
    // create spectral transform object
    EPSGetST(eps, &st);
...
#  endif
  ...
  //MOD TO ALLOW INTERVAL SPECIFICATION use with set_which_eigenpairs(EPS_ALL)
  void
  SolverBase::set_target_interval(const PetscReal &left_bound, const PetscReal &right_bound)
  {
    // find all eigenvalues
    EPSSetWhichEigenpairs(eps,EPS_ALL);
    // set which subinterval of the eigenspectrum to solve for
    PetscErrorCode ierr = EPSSetInterval(eps, left_bound, right_bound);
    AssertThrow(ierr == 0, ExcSLEPcError(ierr));
      
    // required for spectrum slicing
...
#  endif
  ...
  //MOD TO ALLOW INTERVAL SPECIFICATION use with set_which_eigenpairs(EPS_ALL)
    void
    SolverBase::solve(unsigned int *n_converged)
    {
        // set number of eigenvectors to compute
        /*
        PetscErrorCode ierr =
        ...
        */
        ...
        // set the solve options to the eigenvalue problem solver context
        //MODDED WITH PetscErrorCode
        PetscErrorCode ierr = EPSSetFromOptions(eps);
        AssertThrow(ierr == 0, ExcSLEPcError(ierr));
